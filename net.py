...
class Net(nn.Module):
    def __init__(self, params, anchor_is_phrase):
        ...
        if anchor_is_phrase:
            self.lstm = nn.LSTM(params.word_embedding_dim, params.word_hidden_dim, 1, batch_first=True)
        else:
            self.lstm = nn.LSTM(params.vid_embedding_dim, params.vid_hidden_dim, 1, batch_first=True) 
